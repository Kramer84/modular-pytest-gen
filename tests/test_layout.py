...
from modular_pytest_gen.config import LayoutConfig, ProjectConfig
from modular_pytest_gen.layout import LayoutManager


def test_layout_manager_adjacent():
    config = ProjectConfig(layout=LayoutConfig(strategy="adjacent"))
...
def test_layout_manager_invalid_strategy():
    config = ProjectConfig(layout=LayoutConfig(strategy="unknown_strat"))
    manager = LayoutManager(config)
...
def test_layout_manager_external_flat_deep_nesting():
    ...
    assert manager.get_test_file_path(path) == Path(
        "tests/test_otaf_core_solvers_math.py"
    )
