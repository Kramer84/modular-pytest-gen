...
def test_build_system_prompt_rich_context():
    builder = PromptBuilder()
    global_context = {
        "constants": {"BASE_SURFACE_TYPES": '["plane", "cylinder"]', "TIMEOUT": "10"},
        "exceptions": [
            {
    ...
    assert (
        "Class MissingSurfaceTypeKeyError(KeyError): Import via 'otaf.exceptions.MissingSurfaceTypeKeyError'. Doc: Raised when surface type is missing."
        in system_prompt
    )
    assert (
        "Class ComplexMultiBaseError(ValueError, Exception): Import via 'ComplexMultiBaseError'. Doc: None"
        in system_prompt
    )
